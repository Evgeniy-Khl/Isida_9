...
extern uint8_t Y_txt, X_left, checkButt, card;
...
{
  checkButt = 0;
  while(checkButt<2); //ждём 20 милисекунд, для того, чтобы напряжение стабилизировалось
...
uint8_t sd_ini(void)
{
  ...
  sdinfo.type = 0;
	...
	hspi1.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_256; //281.25 kbbs
	HAL_SPI_Init(&hspi1);
	SS_SD_DESELECT();
