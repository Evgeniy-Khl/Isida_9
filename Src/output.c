...
//---- функция ограничения iPart ---------------
  if (pPart > maxVal) iPart[cn] = 0;            // функция ограничения iPart
  else if(pPart > 0)  iPart[cn] = Ud - pPart;   // "антинасыщяющая" поправка
  return Ud;
};
...
uint16_t humidifier(int16_t err, struct eeprom *t){
  ...
  if(t->relayMode&2){
    ...
    result = UpdatePID(err, 2, t);                              // определение длительности ВКЛ. состояния
  ...
  else result = UpdatePID(err, 1, t);                           // для режима 0-НЕТ; 1->по кан.[0]
  return result;
}
