...
void rs485Callback(uint8_t *p_ramdata, uint8_t *p_eepdata){
  ...
  word %=255;
  if(word != rs485Data.RXBuffer[RX_BUFFER_SIZE-1]) return;   // если неверная CRC то выход.
  p_ind = p_eepdata+34; // указатель на eep->identif сетевой номер прибора
  identif = *p_ind;
        ...
        case COMMAND_LINK:         // Есть связь с компьютером
          {
           ...
           for (i=0; i<7; i++){rs485Data.TXBuffer[item] = 0; item++;}    // item: 43-49  (7 byte)
           //----------------
           ...
           p_ind = p_ramdata+18;  // nothing1, nothing2
           for (i=0; i<2; i++){rs485Data.TXBuffer[item] = *p_ind++; item++;}    // item: 59-60  (2 byte)
           for (i=0; i<5; i++){rs485Data.TXBuffer[item] = 255; item++;}    // item: 61-65 /* ext[5] */  (5 byte)
           for (i=0; i<3; i++){rs485Data.TXBuffer[item] = 0; item++;}    // item: 66-68 /* flap[3] */  (3 byte)
           for (i=0; i<2; i++){rs485Data.TXBuffer[item] = 0; item++;}    // item: 69-70 /* kWattHore */
           p_ind = p_ramdata+8;  // pvRH
           for (i=0; i<2; i++){rs485Data.TXBuffer[item] = *p_ind++; item++;}    // item: 71-72
           p_ind = p_ramdata+2;   // pvT[3]
           for (i=0; i<6; i++){rs485Data.TXBuffer[item] = *p_ind++; item++;}    // item: 73-78
           p_ind = p_ramdata+9;  // pvCO2[3]
           for (i=0; i<6; i++){rs485Data.TXBuffer[item] = *p_ind++; item++;}    // item: 79-84
        ...
        case COMMAND_SP_WRITE:
          {
            ...
            if(item){
              ...
              p_ind = p_eepdata+6; // указатель на eep->spRH[1]
        ...
        case COMMAND_KOF_WRITE:
          {
            ...
            HAL_UART_Transmit(&huart3,(uint8_t*)rs485Data.TXBuffer,3,0x1000);
            item = 3;                                                   //  (8 byte)
            p_ind = p_eepdata+30; // указатель на eep->pkoff[0]=25
            *p_ind++ = rs485Data.RXBuffer[item];  //  pkoff[0]
            // ---------- Ti[0]=900 ---------------
            item = 5;
            intVal.data[0] = rs485Data.RXBuffer[item++];
            intVal.data[1] = rs485Data.RXBuffer[item];
            p_ind = p_eepdata+32;   // указатель на eep->ikoff[0]=90
            *p_ind = intVal.val/10; //  ikoff[0]
            item = 7;
            p_ind = p_eepdata+31; // указатель на eep->pkoff[1]=20
            *p_ind++ = rs485Data.RXBuffer[item];  //  pkoff[1]
            // ---------- Ti[1]=900 ---------------
            item = 9;
            intVal.data[0] = rs485Data.RXBuffer[item++];
            intVal.data[1] = rs485Data.RXBuffer[item];
            p_ind = p_eepdata+33; // указатель на eep->ikoff[1]=900
            *p_ind =  intVal.val/10; //  ikoff[1]
            // запись K[2]; Ti[2]; не выполняется
          }; break;
